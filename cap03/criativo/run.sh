...
# Exemplo: ./run.sh build    (Compila o projeto)
# Exemplo: ./run.sh exc3.11  (Executa o binário do exercício 3.11)
# ======================================================================

# ======================================================================
# 🚨 CONFIGURAÇÃO DE VARIÁVEIS POR CAPÍTULO 🚨
# ======================================================================
IMAGEM="raylib-cap03"           # 1. EDITE: Nome da imagem (Ex: raylib-cap03)
DIR="."                         # Contexto do build: diretório atual
# 2. EDITE: Lista de todos os exercícios do capítulo, separados por pipe '|'
EXERCICIOS="exc3.1|exc3.2|exc3.3|exc3.4|exc3.5|exc3.6|exc3.7|exc3.8|exc3.9"
# ======================================================================
...
# ======================================================================
# Usa a variável EXERCICIOS, aceitando qualquer nome listado (ex: exc3.11)
elif [[ "$COMANDO" =~ ^($EXERCICIOS)$ ]]; then

    BINARIO="./bin/$COMANDO"

    echo "Iniciando a execução gráfica de '$COMANDO' (Interativo Mouse/Teclado)..."

    # PASSO 1: Adiciona permissão temporária para acesso ao X Server
    xhost +local:docker 2>/dev/null

    # PASSO 2: Executa o contêiner com acesso gráfico (sem flags de I/O)
    docker run --rm \
               --privileged \
               -e DISPLAY=$DISPLAY \
...
# ======================================================================
    ...
    echo "  ./run.sh <exercicio>  -> Executa um exercício compilado (ex: ./run.sh exc3.1)."
