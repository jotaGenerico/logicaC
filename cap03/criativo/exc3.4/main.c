...
int main( void ) {
    ...
    while ( !WindowShouldClose() ) {
        ...
        for (int linha = 0; linha < 400 / 20; linha++){

            for (int coluna = 0; coluna < 400 / 20; coluna++){

                // calcula posiçao quadrado
                int x = coluna * quadrado;
                int y = linha * quadrado;

                // pula cor
                Color cor = ((linha + coluna) % 2 == 0) ? BLACK : WHITE;

                // quadrado
                DrawRectangle(x, y, quadrado, quadrado, cor);
            }
        }
